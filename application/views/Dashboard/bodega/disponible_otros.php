		...
		<tbody id="showdata">
			<?php foreach ($detalle as $key ) { ?>
				<tr>
					...
					<td><?php echo empty($key->fecha_ingreso) ? '<span style= "color:red">no disponible</span>' : $key->tipo ?></td>
					<td><?php echo empty($key->tipo) ? '<span style= "color:red">no disponible</span>' : $key->fecha_ingreso ?></td>
					<td><a href="<?php echo base_url();?>validar-otro/<?php echo $key->serial;?>" class="btn btn-success item-view"><i class="fa fa-edit" aria-hidden="true"></i></a></td>
				</tr>
			<?php } ?>
