...
<body>
	...
	<div class="row">
		...
		<table id="tabla" class="table table-dark table-hover">
			...
			<tbody>
				<?php foreach ($detalle as $d => $key ) {?>
				<tr>
					<td scope="row"><?php echo $d+1?></td>
					<td scope="row"><?php echo $key->fecha_cambio ?></td>
					<td scope="row"><?php echo $key->codigo_id ?></td>
					<td scope="row"> <a href="<?php echo base_url('detalle-prestamo/'.$id=$key->id_cambio);?>" class="btn btn-success item-view" ><i class="fa fa-eye" aria-hidden="true"></i></a> </td>
				</tr>
				<?php } ?>
